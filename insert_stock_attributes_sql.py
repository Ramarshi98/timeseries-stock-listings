...
try:
    ...
    # Create the stock_attributes table if it doesn't exist
    create_table_query = '''
    CREATE TABLE IF NOT EXISTS stock_attributes (
        ...
        value VARCHAR(255),
        CONSTRAINT fk_symbol FOREIGN KEY (symbol) REFERENCES securities(symbol) ON DELETE CASCADE
    );
    '''
    cursor.execute(create_table_query)
    
    # Insert the data into stock_attributes table
    for record in data:
        symbol = record["symbol"]
        attributes = record["attributes"]
        
        for attribute, value in attributes.items():
            # Insert each attribute as a new row
            insert_query = '''
            INSERT INTO stock_attributes (symbol, attribute, value)
            VALUES (%s, %s, %s)
            '''
            cursor.execute(insert_query, (symbol, attribute, value))
    
    # Commit the transaction
    conn.commit()
    
    print(f"Successfully inserted attributes for {len(data)} securities into the 'stock_attributes' table.")
